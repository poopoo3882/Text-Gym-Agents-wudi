...
def get_chat(client, messages: list, api_type: str = "azure", model: str = "gpt-35-turbo", temperature: float = 0.0, max_tokens: int = 256, seed: int = 1, stop_strs: Optional[List[str]] = None, is_batched: bool = False) -> str:
    ...
    if api_type == "azure":
        ...
        llm = ChatOpenAI(base_url=f'http://localhost:{self.args.port}/v1',api_key='ollama',model='llama3.1')
        ...
        llm = ChatOpenAI(base_url=f'http://localhost:{self.args.port}/v1',api_key='ollama',model='gemma:7b')
        response = llm.invoke(messages)
        usage={'token': response.response_metadata['token_usage']['total_tokens'], 'cost': 0}
        response=response.content
        return response, usage
        ...
        llm = ChatOpenAI(base_url=f'http://localhost:{self.args.port}/v1',api_key='ollama',model='qwen2.5:7b')
        response = llm.invoke(messages)
        usage={'token': response.response_metadata['token_usage']['total_tokens'], 'cost': 0}
        response=response.content
        return response, usage
