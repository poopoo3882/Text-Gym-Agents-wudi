...
class Guidance_Generator():
    def __init__(self,logfile="",args=None):
        self.args = args
        self.seed = args.seed
        with open("/home/wudi/Text-Gym-Agents-wudi/distillers/guidance_summary_few_shot_examples.txt", 'r') as f:
    ...
    def generate_from_file(self, client, file_path,max_step_num=200):
        ...
        for traj in data: 
            ...
            summary = self.generate_summary(client, traj_text, mem)
            mem.append(summary)
        return mem
    ...
    def generate_summary(self, client, traj, post_memory):
        query = self._generate_summary_query(traj, post_memory)
        summary = get_chat(client, query, api_type=self.args.api_type, model=self.args.gpt_version, )
        logger.info(f'[Reflexion Memory]The summary prompt is: {query}.')
        logger.info(f'[Reflexion Memory]The summary response is: {summary}.')
        return summary
    
    def generate_insight(self, client, post_memory):
        ...
        insight = get_chat(client, query, api_type=self.args.api_type, model=self.args.gpt_version, )
    ...
    def generate_suggestion(self, client, game_description, goal_description, action_description, pre_memory, post_memory, insight, max_num_trials):
        ...
        suggestion = get_chat(client, query, api_type=self.args.api_type, model=self.args.gpt_version, )
    ...
    def generate(self, client, traj, memory, max_len_mem=5):
        ...
        reflection = get_chat(client, reflection_query, api_type=self.args.api_type,  seed=self.seed, model=self.args.gpt_version, )
